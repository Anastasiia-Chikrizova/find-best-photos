...
> **Важно:** `R2_ACCOUNT_ID` — это account id из дашборда (поле «S3 API»
> показывает эндпоинт `https://<account_id>.r2.cloudflarestorage.com`).
> `R2_BUCKET` — ИМЯ бакета, не URL. Перепутать легко: account id и Access Key ID
> оба выглядят как 32 hex-символа.

### Диагностика

`GET /r2/health` проверяет коннект и доступность бакета и отдаёт внятную причину
(неверный account id → SSL handshake failure, нет бакета → 404, нет прав → 403):

```bash
curl https://ваш-домен/r2/health
```
